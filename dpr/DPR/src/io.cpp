...
bool ReadFile_ldblock (const string &file_ldblock, vector<vector<double> > &ldblock)
{
	...
	ifstream infile (file_ldblock.c_str(), ifstream::in);
	if (!infile) {cout<<"error! fail to open ldblock file: "<<file_ldblock<<endl; return false;}
	...
	gsl_matrix *ld=gsl_matrix_alloc (10, 3);
	for (vector<int>::size_type i=0; i<10; ++i) {
		for (size_t j=0; j<3; ++j) {
			gsl_matrix_set (ld, i, j, (ldblock)[i][j]);
			cout<<gsl_matrix_get (ld,i,j)<<" ";
		}
		cout<<endl;
	}*/
...
//		geno_var=geno_mean*(1-geno_mean*0.5);
		...
		for (size_t i=0; i<ni_total; ++i) {
			if (gsl_vector_get (geno_miss, i)==0) {gsl_vector_set(geno, i, geno_mean);}
              ...
              //zeng ping
		gsl_vector_add_constant (geno, -1.0*geno_mean); //this is the original one
             //gsl_vector_add_constant (geno, 0);
...
//	ifstream infile (file_geno.c_str(), ifstream::in);
	...
	gsl_vector *genotype=gsl_vector_alloc (UtX.rows());
	gsl_vector *genotype_miss=gsl_vector_alloc (UtX.cols());
	...
	int ni_test=UtX.rows();
	int ns_test=UtX.cols();
...
bool ReadFile_bed (const string &file_bed, vector<int> &indicator_idv, vector<int> &indicator_snp, MatrixXd UtX, gsl_matrix *K, const bool calc_K)
{
	...
	size_t ni_test=UtX.rows();
	size_t ns_test=UtX.cols();
...
bool ReadHeader (const string &line, HEADER &header)
{
  ...
  string a1_ptr[]={"a1","A1","allele1","ALLELE1","Allele1"};
  set<string> a1_set(a1_ptr, a1_ptr+5);
  string a0_ptr[]={"a0","A0","allele0","ALLELE0","Allele0","a2","A2","allele2","ALLELE2","Allele2"};
...
{
  vec_cat.clear(); vec_ni.clear(); vec_var.clear(); vec_z2.clear();
  ...
  while (!safeGetline(infile, line).eof()) {
    ...
    if (mapRS2var.count(rs)!=0 && (mapRS2cat.size()==0 || mapRS2cat.count(rs)!=0) && zsquare!=0) {
      //obtain var_x
      if (header.var_col==0) {
	if (header.af_col!=0) {
	  var_x=2.0*af*(1.0-af);
...
void Calcq (const int k_mode, const size_t n_block, vector<size_t> &vec_cat, vector<size_t> &vec_ni, vector<double> &vec_var, vector<double> &vec_z2, gsl_matrix *Vq, gsl_vector *q, gsl_vector *s)
{
  ...
  for (size_t i=0; i<vec_cat.size(); i++) {
    ...
    //compute q and s
    vec_q[cat]+=(zsquare-1.0)*var_x/(double)n_total;
    vec_s[cat]+=var_x;
    n_snps[cat]++;
  }
