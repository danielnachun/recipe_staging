...
l_min(1e-7), l_max(1e5), n_region(10),p_nr(0.001),em_prec(0.0001),nr_prec(0.0001),em_iter(10000),nr_iter(100),crt(0),
...
//zeng ping DPR
//zeng ping DPR
tp(10),// sampling likely unimportant SNPs once every T iterations
n_k(4),// number of truncated components
DIC1(0),// compute DIC
mnk(6),// maximum number of truncated components for adaptive selection
pD1(0),// compute pD
pcutoff(500),// obtain the top M marginally significant SNPs
LBO(0),// compute ELBO
sp(0.1),//specify iterations for selecting the optimal nk in the adaptive DPR
//zeng ping DPR
//zeng ping DPR
BIC1(0),DIC2(0),pD2(0),BIC2(0),mixture_no(0),
...
void PARAM::ReadFiles (void)
{
	...
	//read LDblock files
	if (!file_ldblock.empty() ) {
		if (ReadFile_ldblock (file_ldblock, ldblock)==false) {error=true;}
	}
...
void PARAM::CheckParam (void)
{
	...
	if (a_mode!=1 && a_mode!=2 && a_mode!=3 && a_mode!=4 && a_mode!=5 && a_mode!=11 && a_mode!=12 && a_mode!=13 && a_mode!=14 && a_mode!=15 && a_mode!=16 && a_mode!=21 && a_mode!=22 && a_mode!=25 && a_mode!=26 && a_mode!=27 && a_mode!=28 && a_mode!=31 && a_mode!=41 && a_mode!=42 && a_mode!=43 && a_mode!=51 && a_mode!=52 && a_mode!=53 && a_mode!=54 && a_mode!=61 && a_mode!=62 && a_mode!=71)
	{cout<<"error! unknown analysis mode: "<<a_mode<<". make sure -gk or -eigen or -dpr or -predict is sepcified correctly."<<endl; error=true;}
	...
	if (!file_ref.empty()) {
		...
		str=file_ref+".V.txt";
		if (stat(str.c_str(),&fileInfo)==-1) {cout<<"error! fail to open .V.txt file: "<<str<<endl; error=true;}
	...
	if (a_mode==61 || a_mode==62) {
	  if (!file_pheno.empty()) {
	    ...
	    if ( (file_mstudy.empty() && file_study.empty()) || (file_mref.empty() && file_ref.empty() )  ) {
	      cout<<"error! either phenotype/kinship files or study/ref mstudy/mref files are required."<<endl; error=true;
	    }
	  }
	}
...
void PARAM::ReadGenotypes (MatrixXd UtX, gsl_matrix *K, const bool calc_K) {
	string file_str;
...
void JacknifeGtoS (const gsl_matrix *G, gsl_matrix *S, gsl_matrix *Svar) {
  ...
  for (size_t i=0; i<n_vc; i++) {
	...
	if ( (si[i][t]-sum_Ki[i][t])==0 || (si[j][t]-sum_Ki[j][t])==0) {
	  d=0;
	} else {
...
void PARAM::CalcS (gsl_matrix *S, gsl_matrix *Svar, gsl_matrix *V, gsl_vector *ns)  {
  string file_str;
  ...
  if (!file_bfile.empty() ) {
    file_str=file_bfile+".bed";
    if (PlinkKin (file_str, indicator_idv, indicator_snp, a_mode-24, d_pace, mapRS2cat, mapRS2var, snpInfo, G, ns)==false) {error=true;}
  } else {
    file_str=file_geno;
    if (BimbamKin (file_str, indicator_idv, indicator_snp, a_mode-24, d_pace, mapRS2cat, mapRS2var, snpInfo, G, ns)==false) {error=true;}
  }
  ...
  /*
  //use half of the sample to provide adjustment for the matrix V
  size_t ni_sub=ni_test/2;
  gsl_matrix *Gsub=gsl_matrix_alloc (ni_sub, n_vc*ni_sub);
  gsl_matrix *Vsub=gsl_matrix_alloc (V->size1, V->size2);
  ...
  vector<size_t> idv_order, idv_sample;
  for (size_t i=0; i<ni_test; i++) {
    idv_order.push_back(i);
  }
  for (size_t i=0; i<ni_sub; i++) {
    idv_sample.push_back(0);
  }

  //run 10 sub-samples of half the size, and compute an average value of V
  size_t n_rep=10;
  gsl_matrix_set_zero(Vsub);
  for (size_t t=0; t<n_rep; t++) {
    //subsample idvs to form a sub matrix of G
    gsl_ran_choose (gsl_r, static_cast<void*>(&idv_sample[0]), ni_sub, static_cast<void*>(&idv_order[0]), ni_test, sizeof(size_t));
    for (size_t i=0; i<ni_sub; i++) {
      for (size_t j=i; j<ni_sub; j++) {
	for (size_t k=0; k<n_vc; k++) {
	  d=gsl_matrix_get(G, idv_sample[i], k*ni_test+idv_sample[j]);
	  gsl_matrix_set(Gsub, i, k*ni_sub+j, d);
	  if (i!=j) {gsl_matrix_set(Gsub, j, i, d);}
	}
      }
    }

    //center and scale the sub matrices
    for (size_t i=0; i<n_vc; i++) {
      gsl_matrix_view K=gsl_matrix_submatrix(Gsub, 0, i*ni_sub, ni_sub, ni_sub);
      CenterMatrix(&K.matrix);
      d=ScaleMatrix(&K.matrix);
    }

    //compute Vsub
    compKtoV (Gsub, Vslope);
    gsl_matrix_add(Vsub, Vslope);
  }
  gsl_matrix_scale(Vsub, 1.0/(double)n_rep);

  //based on Vsub, and V to fit a linear model to compute the slop for every element in V, and store that in Vslope
  ...
  for (size_t i=0; i<n_vc; i++) {
    for (size_t j=i; j<n_vc; j++) {
      ...
      for (size_t l=0; l<n_vc+1; l++) {
	for (size_t m=0; m<n_vc+1; m++) {
	  //if ( ((i==j) && (j==l)) || ((i==j) && (j==m)) || ((i==l) && (l==m)) || ((j==l) && (l==m)) ) {
	    d=(gsl_matrix_get (V, l, t_ij*(n_vc+1)+m)-gsl_matrix_get (Vsub, l, t_ij*(n_vc+1)+m))/(double)(ni_test-ni_sub);
	    //if (d>0) {
	      gsl_matrix_set (Vslope, l, t_ij*(n_vc+1)+m, d);
	      //}
	      //}
	}
      }
...
void PARAM::WriteMatrix (const gsl_matrix *matrix_U, const string suffix)
{
	...
	///////////////// ZP
	string file_strk;
	file_strk=path_out+"/"+file_out;
	//file_strk+=".";
	//file_strk+=suffix;
	file_strk+=".grm.gz";
	ogzstream kmat (file_strk.c_str(), ogzstream::out);

	kmat.precision(6);
	for(size_t i=0; i<matrix_U->size1; i++) {
       for(size_t j=0; j<=i; j++)  {
		   kmat<<i+1<<'\t'<<j+1<<'\t'<<(int)ns_test<<'\t'<<gsl_matrix_get (matrix_U, i, j)<<endl;
	   }
    }
	kmat.close(); kmat.clear();
	///////////////// ZP
	return;
}
...
// copy the ld block file
 void PARAM::CopyLDblock (gsl_matrix *LDblock0)
{
	size_t no_block = ldblock.size();
	for (vector<int>::size_type i=0; i<no_block; ++i) {
		for (size_t j=0; j<3; ++j) {
			gsl_matrix_set(LDblock0,i,j,(ldblock)[i][j]);
		}
	}
	return;
}
