...
void LDR::Gibbs1 (const gsl_matrix *UtX, const VectorXd Uty, const MatrixXd UtW, const MatrixXd UtG, const VectorXd D, const VectorXd sig_index, const VectorXd sig_subsnp_no, const VectorXd Wbeta0, const VectorXd se_Wbeta0, const VectorXd beta0, double lambda) 
{
	...
	size_t n_j   = UtW.cols();
	VectorXd x_col(n_idv);
	//size_t n_snp_mixt = sig_subsnp_no.size();
	...
	VectorXd Ue(n_idv);
	VectorXd Ub(n_idv);
	VectorXd eigen_bv0 = VectorXd::Zero(n_idv); //for computation of alpha
	VectorXd eigen_bv  = VectorXd::Zero(n_idv); //for computation of alpha
	...
	for (size_t S=0; S<(w_step + s_step); S++)   { 
		...
		for (size_t i=0; i<n_snp; i++)   {
			...
			xy0        = x_col.array()*(y_res - XEbeta).array();
			xy2        = xy0.dot(xy0);
			for (size_t k=0; k<n_k; k++)   {				
				if (k == 0) {
					mik_beta(i,k)  = 0; 
					...
					pikexp1(k)     = pow(mik_beta(i,k),2)/(2*sik2_beta(i,k)) + 0.5*log(sik2_beta(i,k)) - Elogsigmak(k);
				}
				pikexp2(k)     = Elogvk(k) + sumElogvl(k);
				beta_beta(i,k) = gsl_ran_gaussian(rs,sqrt(sik2_beta(i,k))) + mik_beta(i,k);
				logV =  log(sigma2k(k)/sigma2e*xtx(i) + 1);
				//pikexp1(k) = mik_beta(i,k)*xy2/(2*sigma2e) - 0.5*logV;
				//cout << pikexp2(k)+pikexp1(k)<<" ";
				}
		...
		for (size_t k=0; k<n_k; k++)   {
			...
			//cout<<setprecision(0)<<gamma_beta(i,k)<<"   ";
		}
		//cout << endl;
		// multinomial sampling
		Ebeta(i) = ((beta_beta.row(i)).cwiseProduct(gamma_beta.row(i))).sum();
		XEbeta  += x_col*Ebeta(i);
		}
		...
		for (size_t i=0; i<n_idv; i++) {
			...
			eigen_bv0(i) = y_res(i)*sigma2b/(sigma2b*D(i) + 1); 
			}
		...
		A  = (Uty - XEbeta - WEalpha - Utu).dot(Uty - XEbeta - WEalpha - Utu);
		a_e = n_snp/2 + ae;
		b_e = 0.5*(A + 2*be);
		sigma2b = 1/gsl_ran_gamma(rs,n_idv/2,1/(Ub.dot(Utu)));
		////////////////////////////////////

		//double Gn = gamma_beta.rightCols(n_k-1).sum();
		//cout<<S+1<<setprecision(0)<<"  "<<Gn<<"  "<<setprecision(3)<<sigma2e<<endl;
	...
	for (size_t i=0; i<n_idv; i++) {
		gsl_vector_set (gsl_bv, i, eigen_bv(i)/s_step);
	}
