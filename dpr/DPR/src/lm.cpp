...
//ZP
void LM::Call_lm_Pvalue(vector<double> *lm_pvalue)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		(*lm_pvalue).push_back(sumStat[t].p_wald);
	t++;
	}
}


void LM::Call_lm_Position(vector<int> *lm_Position)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		(*lm_Position).push_back(snpInfo[i].base_position);
		t++;
	}
}

void LM::Call_lm_Chr(vector<int> *lm_Chr)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		int chr = stod(snpInfo[i].chr);
		(*lm_Chr).push_back(chr);
		t++;
	}
}

void LM::Call_lm_Chr_Position(vector<int> *lm_Chr, vector<int> *lm_Position)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		...
		(*lm_Chr).push_back(chr);
		(*lm_Position).push_back(snpInfo[i].base_position);
		//cout<<chr<<" "<<snpInfo[i].base_position<<endl;
		t++;
	}
}

void LM::Call_lm_Beta(vector<double> *lm_beta)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		(*lm_beta).push_back(sumStat[t].beta);
		t++;
		}
}
	
void LM::Call_lm_Se(vector<double> *lm_se)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		(*lm_se).push_back(sumStat[t].se);
		t++;
		}
}

void LM::Call_lm_Rs(vector<string> *lm_rs)
{
	size_t t=0;
	for (size_t i=0; i<snpInfo.size(); ++i) {
		if (indicator_snp[i]==0) {continue;}
		(*lm_rs).push_back(snpInfo[i].rs_number);
		t++;
	}
}
//ZP
