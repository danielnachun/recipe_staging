...
//then, use lm file to compute the q vector (and the average sample size)
//finall, read in multiple S, q and var_q to compute the variance components
void VC::CalcVCss ()
{
  /*
  bool flag_priorscale=false;
  if (a_mode==62) {flag_priorscale=true;}

  map<string, size_t> mapRS2cat;
  ...
  vector<size_t> vec_n;
  vector<double> vec_cm, vec_bp;

  if (file_mq.empty()) {
    //category from 0 to k-1
    if (!file_cat.empty() ) {ReadFile_cat (file_cat, mapRS2cat, n_vc);}
  } else {
    CountSingleFileLines (file_mq, n_vc);
  }
  cout<<"## number of variance components = "<<n_vc<<endl;

  gsl_matrix *S_mat=gsl_matrix_alloc (n_vc, n_vc);
  gsl_matrix *Svar_mat=gsl_matrix_alloc (n_vc, n_vc);
  gsl_matrix *Si_mat=gsl_matrix_alloc (n_vc, n_vc);
  gsl_matrix *tmp_mat=gsl_matrix_alloc (n_vc, n_vc);
  gsl_vector *q_vec=gsl_vector_alloc (n_vc);
  gsl_vector *qvar_vec=gsl_vector_alloc (n_vc);
  gsl_vector *s_vec=gsl_vector_alloc (n_vc);
  ...
  //if q and S files are not provided
  //then first calculate q and S
  if (file_mq.empty()) {
    ReadFile_cor (file_cor, setSnps, vec_rs, vec_n, vec_cm, vec_bp, mapRS2in, mapRS2var);
    ReadFile_beta (flag_priorscale, file_beta, mapRS2cat, mapRS2in, mapRS2var, mapRS2nsamp, q_vec, qvar_vec, s_vec, ni_total, ns_total);
    ReadFile_cor (file_cor, vec_rs, vec_n, vec_cm, vec_bp, mapRS2cat, mapRS2in, mapRS2var, mapRS2nsamp, crt, window_cm, window_bp, window_ns, S_mat, Svar_mat, qvar_vec, ni_total_ref, ns_total_ref, ns_test, ns_pair);

    cout<<"## number of analyzed individuals in the sample = "<<ni_total<<endl;
    cout<<"## number of analyzed individuals in the reference panel = "<<ni_total_ref<<endl;
    cout<<"## number of total SNPs in the sample = "<<ns_total<<endl;
    cout<<"## number of total SNPs in the reference panel = "<<ns_total_ref<<endl;
    cout<<"## number of analyzed SNPs = "<<ns_test<<endl;
    cout<<"## number of analyzed SNP pairs = "<<ns_pair<<endl;
  } else {
    ReadFile_mq (file_mq, s_vec, q_vec, qvar_vec);
    ReadFile_ms (file_ms, S_mat, Svar_mat);
  }

  WriteFile_qs (s_vec, q_vec, qvar_vec, S_mat, Svar_mat);

  //write out files to save q_vec, qvar_vec, S_mat, Svar_mat
  ...
  //calculate sigma2snp and pve
  gsl_blas_dgemv (CblasNoTrans, 1.0, Si_mat, q_vec, 0.0, sigma2persnp);
  gsl_vector_memcpy(pve, sigma2persnp);
  gsl_vector_mul(pve, s_vec);
  ...
  for (size_t i=0; i<n_vc; i++) {
    for (size_t j=i; j<n_vc; j++) {
      d=gsl_matrix_get(Svar_mat, i, j);
      d*=gsl_vector_get(sigma2persnp, i)*gsl_vector_get(sigma2persnp, j);
      if (i==j) {
	//cout<<d<<" "<<gsl_vector_get(qvar_vec, i)<<endl;
	d+=gsl_vector_get(qvar_vec, i);
	gsl_matrix_set(tmp_mat, i, i, d);
	//gsl_matrix_set(tmp_mat, i, i, gsl_vector_get(qvar_vec, i));
      } else {
	gsl_matrix_set(tmp_mat, i, j, d);
      	gsl_matrix_set(tmp_mat, j, i, d);
      }
    }
  }

  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, Si_mat, tmp_mat, 0.0, Svar_mat);
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, Svar_mat, Si_mat, 0.0, tmp_mat);

  for (size_t i=0; i<n_vc; i++) {
    d=sqrt(gsl_matrix_get(tmp_mat, i, i));
    gsl_vector_set(se_sigma2persnp, i, d);
    d*=gsl_vector_get(s_vec, i);
    gsl_vector_set(se_pve, i, d);
  }
  ...
  gsl_matrix_free(Svar_mat);
  gsl_matrix_free(Si_mat);
  gsl_matrix_free(tmp_mat);
  gsl_vector_free(q_vec);
  gsl_vector_free(qvar_vec);
...
//calculate VC with summary level statistics
/*
void VC::CalcVCss ()
{
  bool flag_priorscale=false;
